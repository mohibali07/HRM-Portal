...
class Settings extends MY_Controller
{
	...
	public function create_database_backup()
	{
		...
		if ($this->input->post('type') === 'backup') {
			...
			$prefs = array(
				'format' => 'gzip',
				'filename' => 'backup_' . date('d-m-Y_H_i_s') . '.sql'
			);

			$backup = $this->dbutil->backup($prefs);

			$name = 'backup_' . date('d-m-Y_H_i_s');
			$fname = $name . '.sql.gz';
			$save = 'uploads/dbbackup/' . $fname;
	...
	public function email_template_list()
	{
		...
		foreach ($email_template->result() as $r) {
			...
			$data[] = array(
				'<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_edit') . '"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-template_id="' . $r->template_id . '"><i class="fa fa-pencil-square-o"></i></button></span>',
				$r->name,
				$r->subject,
				$status
			);
