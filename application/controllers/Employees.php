...
class Employees extends MY_Controller
{
	...
	public function dialog_contact()
	{
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if ($user_id != $result[0]->employee_id && !in_array('13', $role_resources_ids)) {
			redirect('dashboard/');
		}
		// End Security Check
		$data = array(
	...
	public function dialog_document()
	{
		...
		$user_id = $session['user_id'];
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if ($user_id != $document[0]->employee_id && !in_array('13', $role_resources_ids)) {
			redirect('dashboard/');
		}
		// End Security Check
		$data = array(
		...
		if ($this->input->post('type') == 'profile_picture') {
			...
			if ($_FILES['p_file']['size'] == 0 && null == $this->input->post('remove_profile_picture')) {
				...
				if (is_uploaded_file($_FILES['p_file']['tmp_name'])) {
					...
					$mime = finfo_file($finfo, $_FILES['p_file']['tmp_name']);
					finfo_close($finfo);
					...
					if (in_array($ext, $allowed) && in_array($mime, $allowed_mimes)) {
						...
						if (getimagesize($_FILES['p_file']['tmp_name']) === false) {
